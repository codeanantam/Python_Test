# Variables and Data Types in Python

## 1. Introduction

In programming, data must be stored so it can be reused later. In Python, we store data inside **variables**.

Variables are one of the most important foundations of programming. Without variables, you cannot perform calculations, take input, or build logic.

## 2. What is a Variable?

A variable is a name given to a value.

It allows us to store information and use it later.

**Example:**

```python
name = "Aarav"
age = 21
```

Here:

- `name` stores text
- `age` stores a number

To display them:

```python
print(name)
print(age)
```

## 3. Rules for Naming Variables

You must follow these rules:

- A variable name can contain letters, numbers, and underscore (`_`)
- It cannot start with a number
- It cannot use Python keywords (like `if`, `for`, `while`)
- Python is case-sensitive (`age` and `Age` are different)

**Valid examples:**

```python
student_name = "Riya"
marks1 = 90
total_score = 450
```

**Invalid examples:**

```python
1marks = 90        # Cannot start with number
class = "BCA"      # 'class' is a keyword
```

## 4. Understanding Dynamic Typing

Python automatically understands the type of data you assign.

**Example:**

```python
x = 10      # integer
x = "Hello" # now string
```

You do not need to declare the data type separately. Python decides it automatically.

This is called **dynamic typing**.

## 5. Core Data Types for Beginners

We focus only on the most important types.

### 5.1 Integer (int)

Whole numbers without decimals.

```python
age = 18
total_marks = 450
```

### 5.2 Float (float)

Numbers with decimal values.

```python
price = 99.99
temperature = 36.5
```

### 5.3 String (str)

Text values written inside quotes.

```python
name = "Kiran"
city = 'Mumbai'
```

Both single and double quotes are allowed.

### 5.4 Boolean (bool)

Represents only two values:

- `True`
- `False`

**Example:**

```python
is_student = True
is_logged_in = False
```

Booleans are mainly used in conditions (which you will learn later).

## 6. Checking Data Type Using `type()`

You can check the type of a variable using `type()`.

**Example:**

```python
age = 22
print(type(age))
```

**Output:**

```
<class 'int'>
```

Another example:

```python
name = "Python"
print(type(name))
```

## 7. Constants in Python

A **constant** is a variable whose value should not change during the program.

Python does not have a built-in constant keyword, but by convention, we use **UPPERCASE** names to indicate that a variable is a constant.

**Example:**

```python
PI = 3.14159
MAX_USERS = 100
APP_NAME = "Code Anantam"
```

> Note: Python will not prevent you from changing these values, but using uppercase signals to other developers that these should remain unchanged.

## 8. Reassigning Variables

You can change the value of a variable anytime.

```python
score = 50
score = 75
print(score)
```

The latest value will overwrite the previous one.

## 9. Multiple Assignment

Python allows assigning multiple variables in one line.

**Example:**

```python
a, b, c = 10, 20, 30
```

Or assigning same value:

```python
x = y = z = 5
```

## 10. Practice Exercises

1. Create a variable `country` and store your country name.
2. Create variables for `price` and `quantity`.
3. Print all variables.
4. Use `type()` to check their types.
5. Create a boolean variable called `is_available`.

**Example practice:**

```python
country = "India"
price = 500
quantity = 3
is_available = True

print(country)
print(type(price))
print(type(is_available))
```
